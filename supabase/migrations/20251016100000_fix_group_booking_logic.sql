-- Step 1: Update the booking creation function to distribute party size.
DROP FUNCTION IF EXISTS create_booking_for_party(text,text,uuid[],integer,date,time);
...
BEGIN
    ...
    -- Loop through tables and create bookings with distributed party sizes
...
-- Step 2: Create a function to safely cancel an entire booking group.
...
-- Step 3: Create a function to handle the complex update logic.
-- This function will cancel the old group and create a new one.
CREATE OR REPLACE FUNCTION update_booking_group(
    p_group_id_to_cancel UUID,
    p_customer_name TEXT,
    p_customer_phone TEXT,
